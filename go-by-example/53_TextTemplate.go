...
func main() {
	// We can create a new template and parse its body from a string. T
	// emplates are a mix of static text and “actions” enclosed
	// in {{...}} that are used to dynamically insert content.
	t1 := template.New("t1")
	...
	// Alternatively, we can use the template.Must function to panic in case
	// Parse returns an error. This is especially useful for templates
	// initialized in the global scope.
	t1 = template.Must(t1.Parse("Value: {{.}}\n"))

	// By “executing” the template we generate its text with specific values
	// for its actions. The {{.}} action is replaced by the value passed as
	// a parameter to Execute.
	...
	// its fields. The fields should be exported to be accessible when
	// a template is executing.
	...
	// false if it’s the default value of a type, such as 0, an empty string, nil pointer, etc.
