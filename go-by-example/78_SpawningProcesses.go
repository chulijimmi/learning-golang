...
func main() {
	...
	dateCmd := exec.Command("ping localhost")
	...
	// Output and other methods of Command will return *exec.Error if there was a
	// problem executing the command (e.g. wrong path), and *exec.ExitError if the
	// command ran but exited with a non-zero return code.
	...
	// if err != nil pattern for all of them. We also only collect the StdoutPipe results,
	...
	// command and argument array, vs. being able to just pass in one command-line string.
