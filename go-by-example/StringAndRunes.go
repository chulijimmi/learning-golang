...
func main() {
	...
	// This loop generates the hex values of all the bytes that
	...
	// Note that the run-time of RuneCountInString dependes on the size of
	// the string, because it has to decode each UTF-8 rune sequentially.
	// Some Thai characters are represented by multiple UTF-8 code points,
	// so the result of this count may be surprising.
	fmt.Println("Rune count:", utf8.RuneCountInString(s))

	// A range loop handles strings specially and decodes each rune along
	// with its offset in the string.
