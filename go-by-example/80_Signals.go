...
func main() {
	...
	// We could receive from sigs here in the main function, but let’s see how this could
	// also be done in a separate goroutine, to demonstrate a more realistic scenario of
	// graceful shutdown.
	done := make(chan bool, 1)

	// This goroutine executes a blocking receive for signals. When it gets one it’ll print it out
	...
	// The program will wait here until it gets the expected signal (as indicated by the
	// goroutine above sending a value on done) and then exit.
